...
export function MonthlyPayments() {
  ...
  return <section className={styles.page}>
    <header><p>Estado de cuenta</p><h1>Pagos por mes</h1><p>Elegí uno o varios meses del mismo proveedor.</p></header>
    <Label htmlFor="payment-provider">Proveedor</Label>
    <Select id="payment-provider" value={provider} onChange={event => { setProvider(event.target.value); setSelected([]); setFilename(''); setMessage('') }}>{providers.map(name => <option key={name}>{name}</option>)}</Select>
    {message && <Alert role="status"><AlertDescription>{message}</AlertDescription></Alert>}
    <div className={styles.layout}><Card><CardHeader><h2>{provider}</h2><p>{rows[0]?.account}</p><p>Saldo pendiente: {money(rows.filter(debt => debt.status !== 'paid').reduce((sum, debt) => sum + debt.amount, 0))}</p>
      <Button variant="outline" disabled={!payable.length} onClick={() => setSelected(allSelected ? [] : payable.map(debt => debt.id))}>{allSelected ? 'Quitar selección' : 'Seleccionar meses pendientes'}</Button>
      ...
      {rows.map(debt => {
        ...
        return <div className={styles.debt} key={debt.id}>
          ...
          <div><Label htmlFor={debt.id}>{debt.month}</Label><Badge variant="outline">{labels[debt.status]}</Badge>{receipt && <small>{receipt.id}</small>}</div>
    ...
    <h2>Comprobantes informados</h2>
    <ReceiptList provider={provider} />
  </section>
}

/** El proveedor revisa un solo comprobante con el desglose de todos sus meses. */
export function ReceiptList({ provider, canReview = false }: { provider: string; canReview?: boolean }) {
  const { receipts } = usePaymentStore()
  const rows = receipts.filter(receipt => receipt.provider === provider)
