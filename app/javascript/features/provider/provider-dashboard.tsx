...
import type { PaymentStatus, ProviderOrderStatus, ProviderSection } from '@/domain/provider'
import { initialDishes, initialOrders, initialPayments, providerDays, providerProfile, reusableDishes } from '@/mocks/provider'
...
export function ProviderDashboard({ email }: Props) {
  ...
  const home = <>
    ...
    {orders.some(order => order.cancellationNotice) && <Alert className={`${styles.alert} ${styles.cancelAlert}`} variant="destructive"><Bell /><div><AlertTitle>Un pedido confirmado fue cancelado</AlertTitle><AlertDescription>Revisá PED-1046 y ajustá tu preparación.</AlertDescription></div><AlertAction><Button variant="outline" size="sm" onClick={() => { setOrderFilter('cancelled'); navigate('orders') }}>Revisar <ChevronRight /></Button></AlertAction></Alert>}
  ...
  const menu = <>
    ...
    <section className={styles.toolbar}><div><CalendarDays /><span><strong>{currentDay.full}</strong><small>{published.includes(day) ? 'Menú publicado' : 'Borrador · todavía no visible'}</small></span></div><div><Button variant="outline" onClick={() => setReuseOpen(value => !value)}><Copy /> Reutilizar</Button><Button onClick={() => { setPublished(current => current.includes(day) ? current : [...current, day]); announce(`Menú del ${currentDay.day} publicado`) }}><Check /> Publicar menú</Button></div></section>
    {reuseOpen && <section className={styles.reuse}><Title eyebrow="Tu biblioteca" title="Reutilizar un plato anterior" action="Cerrar" onAction={() => setReuseOpen(false)} />{reusableDishes.map(dish => <button key={dish.id} onClick={() => { setDishes(current => current.some(item => item.name === dish.name) ? current : [...current, { ...dish, id: Date.now() }]); setReuseOpen(false); announce(`${dish.name} agregado`) }}><span><strong>{dish.name}</strong><small>{dish.description}</small></span><Plus /></button>)}</section>}
    <div className={styles.dishes}>{dishes.map(dish => <Card className={styles.dish} size="sm" key={dish.id} data-disabled={!dish.available}><span className={styles.food}><UtensilsCrossed /></span><div><Badge variant={dish.available ? 'secondary' : 'outline'}>{dish.available ? 'Disponible' : 'No disponible'}</Badge><h2>{dish.name}</h2><p>{dish.description}</p><strong>${dish.price}</strong></div><button className={styles.toggleText} data-enabled={dish.available} onClick={() => setDishes(current => current.map(item => item.id === dish.id ? { ...item, available: !item.available } : item))} role="switch" aria-checked={dish.available}><i />{dish.available ? 'Disponible' : 'Agotado'}</button></Card>)}</div>
